...
namespace FTSISAPB1iService
{
    class TransferFile
    {
        ...
        private static bool getSFTPCredentials()
        {
            ...
            {
                oRSCred = null;
                strQuery = string.Format("SELECT TOP 1 OISS.\"U_FTPHost\", OISS.\"U_FTPUserName\", OISS.\"U_FTPPassword\", OISS.\"U_FTPPort\", " +
                                         "             OISS.\"U_FTPImpPath\", OISS.\"U_FTPExpPath\", OISS.\"U_FTPRetSucPath\",  OISS.\"U_FTPRetErrPath\" " +
                                         "FROM \"@FTOISS\" \"OISS\" " +
                ...
                strSFTPHost = oRSCred.Fields.Item("U_FTPHost").Value.ToString();

                if (string.IsNullOrEmpty(strSFTPHost))
                    return false;

                strSFTPUserName = oRSCred.Fields.Item("U_FTPUserName").Value.ToString();
                strSFTPPassword = oRSCred.Fields.Item("U_FTPPassword").Value.ToString();

                intSFTPPort = Convert.ToInt32(oRSCred.Fields.Item("U_FTPPort").Value.ToString());

                strSFTPImpPath = oRSCred.Fields.Item("U_FTPImpPath").Value.ToString();
                strSFTPExpPath = oRSCred.Fields.Item("U_FTPExpPath").Value.ToString();
                strFTPSucRetPath = oRSCred.Fields.Item("U_FTPRetSucPath").Value.ToString();
                strFTPErrRetPath = oRSCred.Fields.Item("U_FTPRetErrPath").Value.ToString();
        ...
        public static void importSharedFiles(string strProcess, string strStatus, string strFileName)
        {
            string strFilePath, strFileNewPath = "";
